...
$sql = "INSERT INTO golconda (name, email, tickets, date, time, adult_tickets, children_tickets)
        VALUES ('$name', '$email', '$tickets', '$date', '$time', '$adult_tickets', '$children_tickets')";

// Execute the statement
if ($conn->query($sql) === TRUE) {
    // Provide download link for PDF
    $golconda_id = $conn->insert_id;
    echo '<div style="text-align: center; margin-top: 20px;">';
    echo '<p style="font-size: 18px; color: green;">Successfully registered.</p>';
    echo '<a href="downloadgolconda.php?id=' . $golconda_id . '" style="display: inline-block; padding: 10px 20px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px;">Download Ticket</a>';
    echo '</div>';
} else {
