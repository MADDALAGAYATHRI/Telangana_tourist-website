...
if(isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM yadagiri WHERE id = ?");
    $stmt->bind_param("i", $yadagiri_id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf->Cell(0, 10, 'Free Tickets: ' . $row['free_tickets'], 0, 1);
        $pdf->Cell(0, 10, 'VIP Darshan: ' . $row['vip_darshan'], 0, 1);
        
        // Output PDF as download
        $pdf->Output('yadagiri_ticket.pdf', 'D');
    } else {
