...
if(isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM ramoji WHERE id = ?");
    $stmt->bind_param("i", $ramoji_id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf->Cell(0, 10, 'Total Tickets: ' . $row['tickets'], 0, 1);
        $pdf->Cell(0, 10, 'Adult Tickets: ' . $row['adult_tickets'], 0, 1);
        $pdf->Cell(0, 10, 'Child Tickets: ' . $row['children_tickets'], 0, 1);
        $pdf->Cell(0, 10, 'Date: ' . $row['date'], 0, 1);
        $pdf->Cell(0, 10, 'Time: ' . $row['time'], 0, 1);
        
        // Output PDF as download
        $pdf->Output('ramoji_ticket.pdf', 'D');
    } else {
