...
$stmt = $conn->prepare("INSERT INTO reservations (name, email, number_of_tickets, date, time) VALUES (?, ?, ?, ?, ?)");
...
if ($stmt->execute()) {
    ...
    echo '<a href="download_ticket.php?id=' . $reservation_id . '" style="display: inline-block; padding: 10px 20px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px;">Download Ticket</a>';
    echo '</div>';
} else {
